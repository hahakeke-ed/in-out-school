...
const BASELINE_CSV_URL="https://docs.google.com/spreadsheets/d/e/2PACX-1vSW-NQvM_hYkajd1ZYyzWWGbx0qnUFMC9rlWeVLwSla5Xwovivh4Xp9y_bN73fg4Ab4-uidhJP63rzx/pub?gid=1708903695&single=true&output=csv"; // baseline('학년도' 열 지원)
...
// 번역
function tStatus(s){ if(s==='transfer_in') return '전입'; if(s==='transfer_out') return '전출'; if(s==='enrolled') return '재학'; return s||''; }
...
form.addEventListener('submit', async e=>{ e.preventDefault(); const fd=new FormData(form);
  ...
  try{ await fetch('/',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({'form-name':'transfer',...Object.fromEntries(fd.entries())})}); alert('등록 완료!'); e.target.reset(); }
